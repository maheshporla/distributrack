...
export function SalesReport() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      console.error(error);
      setLoadError("Failed to load the sales report");
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Revenue" value={data ? formatINR(data.totalRevenue) : "₹0.00"} icon={DollarSign} isLoading={isLoading} />
      ...
      <div className="rounded-lg border bg-card">
        {loadError ? (
          <ErrorState title="Couldn't load the sales report" description={loadError} onRetry={load} />
        ) : isLoading ? (
